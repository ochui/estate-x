...
class ModelPropertyaddphotos extends Model {
          ...
          $upload_files = array();
			...
			if(!empty($upload_files)) {
				foreach($upload_files as $image) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "album_photos SET album_id = '" .$data['album_id'] . "',image = '" . $this->db->escape('catalog/gallery/'. $image) . "'");
 ...
 public function getTotaladdphotoss() {
      	$query = $this->db->query("SELECT COUNT(*) AS total FROM " . DB_PREFIX . "album_photos as ap Left join " .DB_PREFIX. "album_photo_description AS apd on(ap.album_photos_id=apd.album_photos_id) WHERE apd.language_id='".$this->config->get('config_language_id')."' AND apd.language_id='".$this->config->get('config_language_id')."' AND ap.album_id>0");
		return $query->row['total'];
	}	
}
?>